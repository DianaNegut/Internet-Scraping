...
def create_welcome_frame(root, show_main_screen_callback, show_email_notification_page_callback):
    ...
    text = "Bine ați venit la Alerte Prețuri!"
...
def create_input_frame(root, add_product_callback):
    ...
    entry_product_name = ctk.CTkEntry(frame_input, width=300)
    ...
    entry_target_price = ctk.CTkEntry(frame_input, width=100)
...
def add_product_frame(frame_products, product_name, target_price, listbox_products):
    frame_product = ctk.CTkFrame(frame_products, corner_radius=10)
    frame_product.pack(padx=5, pady=5, fill="x")

    label_product = ctk.CTkLabel(frame_product, text=product_name, font=("Arial", 12))
    label_product.pack(side="left", padx=5, pady=5)

    label_target = ctk.CTkLabel(frame_product, text=f"Preț țintă: {target_price} lei", font=("Arial", 12))
    label_target.pack(side="left", padx=5, pady=5)

    listbox_products.insert("end", product_name)
